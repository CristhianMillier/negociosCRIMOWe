...
<div class="card-body">
    @can('crear-categoria')
    <a href="{{ route('categorias.create') }}"><button type="button" class="btn btn-outline-info">Añadir nueva
            Categoría</button></a>
    @endcan
    <h5 class="card-title mt-5"><i class="fa-solid fa-table-cells"></i> Tabla Categorias</h5>
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group" role="group">
                            ...
                            <form action=" {{ route('categorias.edit', ['categoria'=>$item]) }}">
                                @csrf
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                @if ($item->estado == 1)
                                ¿SEGURO QUE QUIERES ELIMINAR LA CATEGORÍA - {{ $item->nombre }}?
                                @else
                                ¿SEGURO QUE QUIERES RESTAURAR LA CATEGORÍA - {{ $item->nombre }}?
                                @endif
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                                <form action=" {{ route('categorias.destroy', ['categoria'=>$item->id]) }}"
                                    method="post">
                                    @method('DELETE')
                                    @csrf
